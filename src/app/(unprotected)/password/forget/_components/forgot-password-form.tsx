...
export function ForgetPasswordForm() {
  ...
  async function onSubmit(values: ForgotPasswordDataForm) {
    ...
    try {
      ...
      if (response.isOk) {
        form.reset();
        customToast.success(translation(TRANSLATIONS_KEYS_2.LOGIN.MESSAGES.RESET_LINK_SENT));
      } else
    ...
    <Card className="w-full max-w-lg bg-transparent shadow-none border-0">
      <CardHeader>
        ...
        <CardDescription className="text-xs text-center w-3/4 mx-auto">
          {translation(TRANSLATIONS_KEYS_2.LOGIN.MESSAGES.INSERT_YOUR_EMAIL)}
        </CardDescription>
      </CardHeader>
        ...
        <Form {...form}>
          <form id="forget-password-form" onSubmit={form.handleSubmit(onSubmit)}>
            <InputTextField
              ...
              disabled={isPending}
              required
              placeholder={translation(TRANSLATIONS_KEYS_2.LOGIN.PLACEHOLDERS.EMAIL)}
            />
          </form>
        </Form>
      </CardContent>
