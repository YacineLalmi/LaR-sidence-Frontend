...
import { deleteOfferTypeAction } from "@/actions/offer-types/delete.action";
...
export default function DeleteOfferTypeDialog({ offerType }: Props) {
  ...
  async function onConfirm(id: number) {
    setIsPending(true);
    try {
      const response = await deleteOfferTypeAction(id);
      setIsPending(false);
      if (response.isOk) {
  ...
  return (
    <DeleteConfirmationDialog
      title={translation(TRANSLATIONS_KEYS.SETTINGS.OFFERS.TYPES.DELETE_TEXT, { id: offerType.id })}
      isOpen={isDeleteOpen}
      setIsOpen={setIsDeleteOpen}
      onConfirm={(e: React.MouseEvent<HTMLButtonElement>) => {
        e.preventDefault();
        e.stopPropagation();
        onConfirm(offerType.id);
      }}
