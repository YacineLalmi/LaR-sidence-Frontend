...
import { deleteOfferStatusAction } from "@/actions/offer-status/delete.action";
...
export default function DeleteOfferStatusDialog({ offerStatus }: Props) {
  ...
  async function onConfirm(id: number) {
    setIsPending(true);
    try {
      const response = await deleteOfferStatusAction(id);
      setIsPending(false);
      if (response.isOk) {
  ...
  return (
    <DeleteConfirmationDialog
      title={translation(TRANSLATIONS_KEYS.SETTINGS.OFFERS.STATUS.DELETE_TEXT, { id: offerStatus.id })}
      isPending={isPending}
      isOpen={isDeleteOpen}
