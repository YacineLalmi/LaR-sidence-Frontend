...
export default function BienStatusFilter() {
  ...
  const handleFiltering = useCallback((event: FormEvent<HTMLFormElement>) => {
    ...
    console.log(formData.get("nom_status"));
    ...
    <>
      <div className="flex items-center gap-2 justify-between w-full p-0">
        <div className="flex w-full gap-2">
          <form className="relative w-1/2" onSubmit={handleSearching} id="bien-status-search">
      ...
      <Drawer open={isFilterModalOpen} direction="right">
        <DrawerContent className="p-5">
          ...
          <form onSubmit={handleFiltering} id="bien-status-filter" className="flex flex-col gap-2">
            <CustomInput id="nom_status" name="nom_status" placeholder="Nom status" label="Nom status" />
            <CustomInput id="nom" name="nom" placeholder="Nom" label="Nom" />
          </form>
          <DrawerFooter>
            <Button form="bien-status-filter" type="submit">
              Submit
            </Button>
          </DrawerFooter>
        </DrawerContent>
