...
import { createClientSourceAction } from "@/actions/client-sources/create.action";
...
export default function CreateClientSourceForm() {
  ...
  async function onSubmit(values: ClientSourceForm) {
    setIsPending(true);
    try {
      const response = await createClientSourceAction(values);
      setIsPending(false);
      if (response.isOk) {
        router.push("/settings/clients/sources");
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit, onInvalid)} className="space-y-8 grid grid-cols-2 gap-5">
        <InputTextField
          ...
          name="code"
          label={t("settings.clientSources.form.label.code")}
          disabled={isPending}
          required
          placeholder={t("settings.clientSources.form.placeholder.code")}
        />
        <InputTextField
          control={form.control}
          name="name"
          label={t("settings.clientSources.form.label.name")}
          disabled={isPending}
          required
          placeholder={t("settings.clientSources.form.placeholder.name")}
        />
        <InputTextArea
          control={form.control}
          name="description"
          label={t("settings.clientSources.form.label.description")}
          disabled={isPending}
          required
          placeholder={t("settings.clientSources.form.placeholder.description")}
        />
