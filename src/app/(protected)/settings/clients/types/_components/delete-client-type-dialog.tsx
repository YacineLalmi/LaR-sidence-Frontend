import { deleteClientTypeAction } from "@/actions/client-types/delete.action";
...
export default function DeleteCientTypeDialog({ clientType }: Props) {
  ...
  async function onConfirm(id: number) {
    setIsPending(true);
    try {
      const response = await deleteClientTypeAction(id);
      setIsPending(false);
      if (response.isOk) {
