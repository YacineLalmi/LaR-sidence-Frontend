...
import getDemandStatsActions from "@/actions/statistics/get-demand-stats.action";
...
const TITLE_MAP: Record<string, string> = {
  ...
  priority: "Nombre de demandes",
  status: "Nombre de demandes",
  creation: "Nombre de demandes",
};

const DESCRIPTION_MAP: Record<string, string> = {
  type: "Typologie des demandes",
  source: "Origine des demandes",
  creation: "Date de création",
  status: "État des demandes",
  priority: "Niveau de priorité",
};

export function DemandStatsChart() {
  ...
  const filters: DemandFilterStatsForm = React.useMemo(
    () => ({
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await getDemandStatsActions(currentGroupBy, filters);
        if (response) {
          setData(response);
        }
      } catch (error) {
  ...
  // Check if we should use bar chart (for creation grouping)
        ...
        <div className="flex flex-col gap-2">
          ...
          <Select value={currentGroupBy} onValueChange={handleGroupByChange} disabled={isPending}>
            ...
            <SelectContent>
              ...
              <SelectItem value="priority">Par Priority</SelectItem>
