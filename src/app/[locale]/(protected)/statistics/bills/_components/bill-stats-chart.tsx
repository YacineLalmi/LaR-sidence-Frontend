...
import getBillingStatsActions from "@/actions/statistics/get-bill-stats.action";

const COLORS = ["#A7F3D0", "#FDE68A", "#BFDBFE", "#DDD6FE", "#FCA5A5", "#F9A8D4"];
const BAR_COLOR = "#6366F1"; // Indigo color for bar chart

export function BillStatsChart() {
  ...
  const filters: BillingFilterStatsForm = React.useMemo(
    () => ({
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await getBillingStatsActions(currentGroupBy, filters);
        if (response) {
          setData(response);
        }
      } catch (error) {
    ...
    <Card className="w-full rounded-3xl border-none bg-[#FDFCF9] shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="space-y-1">
          ...
          <CardTitle className="text-2xl font-bold">Répartition des factures</CardTitle>
