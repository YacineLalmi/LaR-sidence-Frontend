...
import getBienDistributionActions from "@/actions/statistics/get-bien-distribution.action";
...
export function BienDistributionChart() {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await getBienDistributionActions(currentGroupBy);
        if (response) {
          setData(response);
        }
      } catch (error) {
    ...
    <Card className="w-full rounded-3xl border-none bg-[#FDFCF9] shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="space-y-1">
          ...
          <CardDescription>Consultez la Répartition des biens par {currentGroupBy}.</CardDescription>
        </div>
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value="exclusivity">Par Exclusivité</SelectItem>
          </SelectContent>
        </Select>
      </CardHeader>
