...
export default function NotificationSettingsForm({ initialSettings }: { initialSettings: NotificationSetting[] }) {
  ...
  const otherRows = useMemo(() => settings.filter((s) => !CATEGORY_SET.has(s.key) && !RULE_SET.has(s.key)), [settings]);
  ...
  const renderRuleBlock = (cat: NotificationCategoryId, ruleKey: NotificationRuleKey) => {
    ...
    if (!ruleSetting || !isRuleKey(ruleKey)) {
      return (
        <div
          key={ruleKey}
          className="rounded-xl border border-dashed border-amber-200/80 bg-amber-50/40 p-4 text-sm text-muted-foreground dark:bg-amber-950/20"
        >
          <span className="font-mono text-xs">{ruleKey}</span>
          <span className="mx-1">—</span>
          {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.MISSING_RULE_HINT)}
        </div>
      );
    }
    ...
    return (
      <div
        ...
        className={cn(
          ...
          ruleOn
            ? "border-stone-200/90 bg-stone-50/50 dark:border-stone-800 dark:bg-stone-950/35"
            : "border-stone-200/60 bg-muted/20 opacity-90 dark:border-stone-800/80",
        )}
      >
          ...
          <div className="flex shrink-0 flex-col items-end gap-1 sm:pt-0.5">
            <div className="flex items-center gap-2">
              <Label htmlFor={applyId} className="text-xs font-medium text-foreground cursor-pointer whitespace-nowrap">
                {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.RULE_APPLY)}
              </Label>
              <Switch id={applyId} checked={ruleOn} onCheckedChange={(v) => setSetting(ruleKey, { enabled: v })} />
            </div>
            <p className="max-w-[220px] text-right text-[11px] leading-tight text-muted-foreground">
              {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.RULE_APPLY_HINT)}
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 space-y-1.5 pl-0 sm:pl-7">
          <Label htmlFor={inputId} className="text-xs font-medium text-muted-foreground">
            {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.VALUE)}
          </Label>
          <Input
            id={inputId}
            disabled={!ruleOn}
        ...
        <Button
          ...
          onClick={() =>
            startTransition(async () => {
              ...
              if (!res.isOk) {
                toast.error(res.errorMessage || translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SAVE_ERROR));
                return;
              }
              toast.success(translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SAVED));
              router.refresh();
            })
          }
          disabled={isPending}
        >
          {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SAVE)}
        </Button>
      </div>
      ...
      <section className="space-y-5">
        <header className="space-y-1.5">
          <h2 className="text-lg font-semibold tracking-tight">
            {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SECTIONS.CATEGORIES)}
          </h2>
          <p className="text-sm text-muted-foreground max-w-3xl leading-relaxed">
            {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SECTIONS.CATEGORIES_HINT)}
          </p>
        </header>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {NOTIFICATION_CATEGORIES.map((cat) => {
              ...
              return (
                <Card key={cat} className="border-dashed border-2 rounded-2xl">
                  <CardHeader>
                    ...
                    <CardDescription>
                      {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.MISSING_CATEGORY_HINT)}
                    </CardDescription>
                  </CardHeader>
                </Card>
              );
            ...
            return (
              <Card
                key={cat}
                className="overflow-hidden rounded-2xl border-stone-200/90 shadow-md dark:border-stone-800"
              >
                <CardHeader className="space-y-1.5 border-b border-stone-100 bg-stone-50/80 pb-4 dark:border-stone-800 dark:bg-stone-900/40">
                  <CardTitle className="text-lg font-semibold leading-tight">
                    {translation(
                      TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.CATEGORIES[cat] as "categories.CONTRACT_EXPIRY",
                    )}
                  </CardTitle>
                  <CardDescription className="text-sm leading-relaxed">
                    {translation(
                      TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.CATEGORYDESCRIPTIONS[
                        cat
                      ] as "categoryDescriptions.CONTRACT_EXPIRY",
                    )}
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-6 pt-6">
                  <div className="space-y-3">
                    <div>
                      <p className="text-sm font-semibold text-foreground">
                        {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNELS_HEADING)}
                      </p>
                      <p className="text-xs text-muted-foreground mt-1">
                        {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNELS_HINT)}
                      </p>
                    </div>
                    <div className="space-y-2.5">
                      {channelRow({
                        ...
                        titleId: `ch-in-${cat}`,
                        title: translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_IN_APP),
                        description: translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_IN_APP_HINT),
                      ...
                      {channelRow({
                        ...
                        titleId: `ch-mail-${cat}`,
                        title: translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_EMAIL),
                        description: translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_EMAIL_HINT),
                        ...
                        <div className="min-w-0 flex-1 space-y-2">
                          <div>
                            <Label className="text-sm font-medium">
                              {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.SCOPE)}
                            </Label>
                            <p className="text-xs text-muted-foreground mt-0.5">
                              {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.SCOPE_HINT)}
                            </p>
                          </div>
                          <Select
                          ...
                          >
                            ...
                            <SelectContent>
                              <SelectItem value="admin">
                                {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.SCOPE_ADMIN)}
                              </SelectItem>
                              <SelectItem value="agent">
                                {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.SCOPE_AGENT)}
                              </SelectItem>
                              <SelectItem value="both">
                                {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.SCOPE_BOTH)}
                              </SelectItem>
                            </SelectContent>
                          </Select>
                        </div>
                  ...
                  <div className="space-y-3">
                    <p className="text-sm font-semibold text-foreground">
                      {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.THRESHOLDS)}
                    </p>
        ...
        <>
          ...
          <section className="space-y-3">
            <h2 className="text-lg font-medium">
              {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.SECTIONS.OTHER)}
            </h2>
                ...
                <Card key={row.key} className="rounded-2xl">
                  ...
                  <CardContent className="grid md:grid-cols-2 gap-4">
                    <div className="space-y-2 md:col-span-2">
                      <Label>{translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.VALUE)}</Label>
                      <Input
                    ...
                    <label className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.ENABLED)}
                      </span>
                    </label>
                    <label className="flex items-center gap-2">
                      <Checkbox
                      ...
                      />
                      <span className="text-sm">
                        {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_IN_APP)}
                      </span>
                    </label>
                    <label className="flex items-center gap-2">
                      <Checkbox
                      ...
                      />
                      <span className="text-sm">
                        {translation(TRANSLATIONS_KEYS_2.SETTINGS.NOTIFICATIONS.FIELDS.CHANNEL_EMAIL)}
                      </span>
                    </label>
                  </CardContent>
                </Card>
