...
export default function AdditionalCharacteristics({ form, isPending = false }: Props) {
  ...
  const [characteristics, setCharacteristics] = useState<ListItem[]>([]);

  const [isCharacteristicsPending, startCharacteristicsTransition] = useTransition();

  useEffect(() => {
    startCharacteristicsTransition(async () => {
      try {
        const results = await getClassificationsListAction(CATEGORIES.CHARACTERISTIC, SCOPES.BIEN);
        setCharacteristics(results);
      } catch (error) {
