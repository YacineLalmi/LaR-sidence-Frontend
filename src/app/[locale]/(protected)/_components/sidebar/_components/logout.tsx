...
export default function Logout() {
  ...
  return (
    <Button
      className={`flex gap-3 hover:bg-accent p-1 px-2 hover:text-primary rounded-lg text-sm w-full justify-start  cursor-pointer`}
      disabled={isPending}
      onClick={handleLogout}
    >
      {isPending ? <Loader className="animate-spin" /> : <LogOut />}
      <span>{translation(TRANSLATIONS_KEYS_2.AUTH.BUTTONS.LOGOUT)}</span>
    </Button>
  );
}
